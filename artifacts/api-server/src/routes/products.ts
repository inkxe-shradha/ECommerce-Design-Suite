...
import { cacheMiddleware, setCacheHeaders } from '../middlewares/cache.js';
...
router.get('/products/search', cacheMiddleware(ONE_MIN), async (req, res): Promise<void> => {
...
router.get('/products/categories', cacheMiddleware(FIVE_MIN), async (_req, res): Promise<void> => {
  const rows = await db
    .select({
...
router.get("/products", cacheMiddleware(2 * ONE_MIN), async (req, res): Promise<void> => {
...
// Deals: cache 2 min
router.get("/products/deals", cacheMiddleware(2 * ONE_MIN), async (req, res): Promise<void> => {
  const rows = await db
    .select()
    .from(productsTable)
...
router.get("/products/popular", cacheMiddleware(2 * ONE_MIN), async (req, res): Promise<void> => {
  ...
  const rows = await db
    .select()
    .from(productsTable)
    .where(gte(sql`CAST(${productsTable.reviewCount} AS numeric)`, minReviews))
...
router.get("/products/:id", cacheMiddleware(2 * ONE_MIN), async (req, res): Promise<void> => {
