...
import { useGetCart, useUpdateCartItem, useRemoveFromCart, useGetCartRecommendations, useAddToCart, getGetCartQueryKey } from "@workspace/api-client-react";
          ...
          {hasItems ? (
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex-1 flex flex-col gap-4">
                {cart.items.map((item) => (
                  <div key={item.product.id} className="bg-white rounded-xl border border-gray-200 p-5 flex gap-5 shadow-sm" data-testid={`cart-item-${item.product.id}`}>
                    <div className="w-32 h-32 bg-gray-50 rounded-lg border border-gray-100 flex items-center justify-center p-2">
                      <img src={item.product.imageUrl || `${import.meta.env.BASE_URL}images/dell-xps-15.jpg`} alt={item.product.name} className="w-full h-full object-contain mix-blend-multiply" />
                    </div>
                    <div className="flex-1 flex flex-col">
                      <div className="flex justify-between items-start mb-1">
                        <Link href={`/product/${item.product.id}`}>
                          <h3 className="font-semibold text-lg text-gray-900 leading-tight hover:text-indigo-600 cursor-pointer">{item.product.name}</h3>
                        </Link>
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden sticky top-24">
                  <div className="p-5 border-b border-gray-100">
                    ...
                    <div className="space-y-3 text-sm text-gray-600 mb-4">
                      ...
                      <div className="flex justify-between">
                        <span>Delivery</span>
                        <span className="font-medium text-green-600">{cart.deliveryFee === 0 ? 'FREE' : formatPrice(cart.deliveryFee)}</span>
                      </div>
                    </div>
              ...
              <div className="bg-gradient-to-r from-indigo-900 to-violet-900 rounded-2xl p-6 relative overflow-hidden shadow-xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 relative z-10">
                  {cartRecs.hybrid.products.map((rec) => (
                    <div key={rec.product.id} className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-4 hover:bg-white/15 transition-colors cursor-pointer group flex items-center gap-4">
                      <div className="w-20 h-20 bg-white rounded-lg p-2 flex-shrink-0 flex items-center justify-center">
                        <img src={rec.product.imageUrl || `${import.meta.env.BASE_URL}images/headphones.jpg`} className="w-full h-full object-contain mix-blend-multiply" />
                      </div>
                      <div className="flex-1">
                        ...
                        <h3 className="font-semibold text-white text-sm leading-tight mb-1 group-hover:text-purple-200 transition-colors line-clamp-2">{rec.product.name}</h3>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {cartRecs.crossSell.products.map((rec) => (
                      <div key={rec.product.id} className="bg-white border border-gray-200 rounded-xl p-3 flex gap-3 hover:shadow-md transition-shadow cursor-pointer">
                        <div className="w-16 h-16 bg-gray-50 rounded flex items-center justify-center p-1">
                          <img src={rec.product.imageUrl || `${import.meta.env.BASE_URL}images/usb-hub.jpg`} className="w-full h-full object-contain mix-blend-multiply" />
                        </div>
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-3">
                    {cartRecs.collaborative.products.map((rec) => (
                      <div key={rec.product.id} className="bg-white border border-gray-200 rounded-xl p-3 flex items-center gap-4 hover:shadow-md transition-shadow cursor-pointer">
                        <div className="w-16 h-16 bg-gray-50 rounded flex items-center justify-center p-1">
                          <img src={rec.product.imageUrl || `${import.meta.env.BASE_URL}images/ssd.jpg`} className="w-full h-full object-contain mix-blend-multiply" />
                        </div>
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {rec.reason && (
                              <span className="text-[10px] font-bold text-orange-600 bg-orange-50 px-1.5 py-0.5 rounded max-w-[120px] truncate">{rec.reason}</span>
                            )}
                          </div>
                        </div>
                        <button 
