...
export default function HomePage() {
  ...
  const { data: homepageRecs } = useGetHomepageRecommendations({ userId: 1 }); // hardcoded userId for mock data
        ...
        <div className="border-b border-gray-100 bg-white">
          <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between overflow-x-auto gap-8">
            {[
              ...
              <button key={cat.name} className="flex flex-col items-center gap-2 min-w-max group cursor-pointer" data-testid={`category-${cat.name}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {deals.map((deal) => (
                <Link key={deal.id} href={`/product/${deal.id}`} className="block">
                  <div className="border border-gray-200 rounded-xl p-4 hover:shadow-lg transition-shadow bg-white relative group cursor-pointer h-full flex flex-col" data-testid={`deal-${deal.id}`}>
                    {deal.discountPct && (
                    ...
                    <div className="h-40 bg-gray-50 rounded-lg mb-4 flex items-center justify-center overflow-hidden">
                      <img src={deal.imageUrl || `${import.meta.env.BASE_URL}images/galaxy-s23.jpg`} alt={deal.name} className="w-full h-full object-contain mix-blend-multiply" />
                    </div>
